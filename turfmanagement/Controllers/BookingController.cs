    ...
    public class BookingController : ControllerBase
    {
        ...
        public IActionResult VerifySlotAvailability([FromBody] SlotCheckDto dto)
        {
            ...
            foreach (var time in dto.SlotTimes)
            {
                ...
                {
                    Console.WriteLine($"❌ Slot {time} is not available.");
                    return Ok(new { status = "Unavailable", message = $"Slot {time} is already booked or under maintenance." });
                }
            }

            Console.WriteLine("✅ All slots are available.");
            return Ok(new { status = "Available", message = "All selected slots are available." });
        }
    }
